...
3. Обновите общую сумму в таблице `Orders` на основе суммы промежуточных итогов по позициям заказа.

### Сценарий 2

Напишите транзакцию, которая обновляет адрес электронной почты клиента в таблице `Customers`. Убедитесь, что обновление является атомарным и не вызывает несоответствий.
...
Написать скрипт/сервис на любом удобном ЯП (использование ORM одобряется). Написать Dockerfile под скрипт/сервис и Docker-compose файл, где указать ваше решение и любую SQL базу.
